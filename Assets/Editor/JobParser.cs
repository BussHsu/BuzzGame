...
public static class JobParser
 {
	[MenuItem("Predicate Production/Parser Jobs")]
	...
	{
		string readPath = string.Format("{0}/Settings/UnitBaseStats.csv", Application.dataPath);
	...
	{
		string readPath = string.Format("{0}/Settings/RaceBonusStats.csv", Application.dataPath);
		string[] readText = File.ReadAllLines(readPath);
		for (int i = 1; i < readText.Length; i++)
	...
	static void ParseGrowthStats(string line) 
	{
		string[] elements = line.Split(',');
		GameObject obj = GetOrCreate(elements[0]);
		Job job = obj.AddComponent<Job>();
		for (int i = 1; i < job.growStats.Length; i++)
